...
const skillGroupsKo = [
  {
    ...
    evidence: [
      'AskOosu: Next.js, TypeScript, RAG, Groq, PostgreSQL 기반 대화형 포트폴리오',
      'mytrip-planner: TypeScript 중심의 AI 여행 플래너 제품 흐름',
      'search-observatory / career-evidence-lab: Next.js·React 기반 실험/분석 도구',
    ],
  },
  {
    ...
    skills: [
      ...
      { name: 'Java', proficiency: 'usable' },
      { name: 'C++20', proficiency: 'learning' },
      { name: 'Boost', proficiency: 'learning' },
      { name: 'CMake', proficiency: 'usable' },
      { name: 'Docker', proficiency: 'usable' },
      { name: 'SQLite', proficiency: 'usable' },
      { name: 'REST APIs', proficiency: 'usable' },
    ],
    evidence: [
      'beneath-the-stack: C++/Assembly로 알고리즘, 메모리, 동시성, 네트워킹, 저장소 내부 구현',
      'memory-atlas-server: C++20/Boost/CMake/Docker 기반 백엔드',
      'Aigram / soundtrack-map-server: Spring Boot·Java 기반 서버 흐름',
    ],
  },
  {
    group: 'Web / Media / Interaction',
    skills: [
      { name: 'Vite', proficiency: 'usable' },
      { name: 'Service Worker', proficiency: 'usable' },
      { name: 'HLS / Video Pipeline', proficiency: 'learning' },
      { name: 'Cloudflare R2', proficiency: 'learning' },
      { name: 'Backblaze B2', proficiency: 'learning' },
      { name: 'GSAP', proficiency: 'usable' },
      { name: 'Three.js', proficiency: 'experimental' },
      { name: 'Lottie', proficiency: 'usable' },
    ],
    evidence: [
      'source-archive: Cloudflare R2, Backblaze B2, HLS preview, Service Worker 기반 영상 아카이브',
  ...
  },
  {
    ...
    skills: [
      ...
      { name: 'CLIP', proficiency: 'learning' },
      { name: 'ViT', proficiency: 'learning' },
      { name: 'PyTorch', proficiency: 'learning' },
      { name: 'OCR APIs', proficiency: 'experimental' },
      { name: 'DeepL', proficiency: 'experimental' },
    ],
    evidence: [
      'AskOosu: AI SDK, Groq, Notion RAG 답변 파이프라인',
      'petlens-ai: ViT 품종 분석과 CLIP 의미 기반 검색을 연결한 멀티모달 웹 서비스',
  ...
  },
  {
    ...
    skills: [
      ...
      { name: 'Swift', proficiency: 'usable' },
      { name: 'iOS', proficiency: 'usable' },
      { name: 'watchOS', proficiency: 'learning' },
    ],
    evidence: [
      'memory-atlas-client: Flutter/Dart 기반 지도형 메모리 아틀라스 클라이언트',
      'soundtrack-map-ios: Swift 기반 장소·음악·기억 연결 앱',
      'health-reflection-ios: iOS/Apple Watch 운동 회고 경험',
    ],
  },
  {
    ...
    skills: [
      ...
      { name: 'SEO Experimentation', proficiency: 'learning' },
    ...
    evidence: [
      'search-observatory: 검색/SEO 실험을 지표와 관찰 기록으로 남기는 분석 도구',
...
const skillGroupsEn = [
  {
    ...
    evidence: [
      'AskOosu: conversational portfolio with Next.js, TypeScript, RAG, Groq, and PostgreSQL',
      'mytrip-planner: TypeScript-heavy AI trip-planning product flow',
      'search-observatory / career-evidence-lab: Next.js and React experimentation/analytics tools',
    ],
  },
  {
    group: 'Backend / Systems',
    ...
    evidence: [
      'beneath-the-stack: C++/Assembly implementations of algorithms, memory, concurrency, networking, and storage internals',
      'memory-atlas-server: C++20/Boost/CMake/Docker backend work',
      'Aigram / soundtrack-map-server: Spring Boot and Java server flows',
    ],
  },
  {
    group: 'Web / Media / Interaction',
    ...
    evidence: [
      'source-archive: video archive with Cloudflare R2, Backblaze B2, HLS previews, and Service Worker loading',
  ...
  },
  {
    ...
    evidence: [
      ...
      'petlens-ai: multimodal web service connecting ViT breed analysis with CLIP semantic search',
  ...
  },
  {
    ...
    evidence: [
      'memory-atlas-client: Flutter/Dart map-first memory atlas client',
      'soundtrack-map-ios: Swift app connecting places, music, and memories',
      'health-reflection-ios: iOS and Apple Watch workout-reflection experience',
    ],
  },
  {
    group: 'Design / UX / Business',
    ...
    evidence: [
      'search-observatory: search/SEO experimentation captured as metrics and observations',
...
export const FAQ_ANSWERS: FaqAnswer[] = [
  ...
  createFaqAnswer({
    ...
    shortAnswer:
      '우수의 현재 스택은 Next.js/TypeScript/RAG 중심의 제품 구현에서 C++ 시스템 실험, Spring Boot 서버, Swift·Flutter 클라이언트, 멀티모달 AI까지 넓어지고 있습니다.',
    defaultAnswer: [
      '우수의 기술 스택은 “무엇을 안다”보다 “어디에 실제로 써봤는가”로 보는 편이 정확합니다.',
      '',
      '최근 GitHub 흐름까지 보면 핵심은 Next.js, React, TypeScript, Tailwind CSS, PostgreSQL, RAG, Groq로 제품을 만들고, 필요에 따라 Spring Boot/Java 서버, C++20/Boost/CMake/Docker 기반 시스템 구현, Swift/iOS·Flutter/Dart 클라이언트, 그리고 CLIP/ViT/PyTorch 같은 멀티모달 AI 실험으로 범위를 확장하는 쪽입니다.',
      '',
      'AskOosu와 mytrip-planner는 AI가 연결된 웹 제품 흐름을, beneath-the-stack과 memory-atlas-server는 더 낮은 레벨의 시스템/백엔드 구현을, petlens-ai는 비전·검색 기반 멀티모달 AI를, soundtrack-map-ios와 health-reflection-ios는 네이티브 모바일 방향을 보여줍니다.',
    ].join('\n'),
    renderSpec: {
      layout: 'skill_cloud',
      density: 'standard',
    ...
    sourceChunkIds: [
      ...
      'project.askoosu.overview',
      'github-project-beneath-the-stack',
      'github-project-mytrip-planner',
      'github-project-source-archive',
      'github-project-petlens-ai',
      'github-project-memory-atlas-server',
      'github-project-health-reflection-ios',
      'github-project-soundtrack-map-ios',
      'project.onjung',
      'project.nomad_market',
      'project.webtoon_translate',
      'project.portfolioh',
    ...
    matchedEntityIds: [
      ...
      'webtoon_translate',
      'portfoli_oh',
      'github:beneath-the-stack',
      'github:mytrip-planner',
      'github:source-archive',
      'github:petlens-ai',
      'github:memory-atlas-server',
      'github:health-reflection-ios',
      'github:soundtrack-map-ios',
    ],
    confidence: 0.97,
  }),
  createFaqAnswer({
    ...
    shortAnswer:
      'Oosu’s current stack centers on Next.js/TypeScript/RAG product work and now extends into C++ systems labs, Spring Boot servers, Swift/Flutter clients, and multimodal AI projects.',
    defaultAnswer: [
      'Oosu’s stack is best understood by looking at where each technology has actually been used.',
      '',
      'Across the latest GitHub evidence, the center of gravity is Next.js, React, TypeScript, Tailwind CSS, PostgreSQL, RAG, and Groq for product work, with expansion into Spring Boot/Java servers, C++20/Boost/CMake/Docker systems work, Swift/iOS and Flutter/Dart clients, and multimodal AI with CLIP, ViT, and PyTorch.',
      '',
      'AskOosu and mytrip-planner show AI-connected web product flow. beneath-the-stack and memory-atlas-server add lower-level systems/backend evidence. petlens-ai shows vision/search AI work, while soundtrack-map-ios and health-reflection-ios show the native mobile direction.',
    ].join('\n'),
    renderSpec: {
      layout: 'skill_cloud',
      density: 'standard',
