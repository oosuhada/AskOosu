...
- `v11`: added FAQ Natural Question Display + Rich Answer Rendering. Existing FAQ items remain the canonical interaction model; when a user clicks a quick FAQ, the chat bubble should show a more natural conversational question, and the answer may render rich visual blocks such as profile cards, project cards, skill chips, image cards, timelines, comparison grids, and contact cards.
...
## 15. FAQ Natural Question Display + Rich Answer Rendering (v11 NEW)

> v11 does not create new dedicated `Me / Projects / Skills / Fun / Contact` knowledge sections. The existing v10 FAQ Answer Cache remains the canonical interaction model. The improvement is to separate the **short quick-question label**, the **natural chat-bubble question**, the **FAQ intent used for routing**, and the **rich visual blocks used to render the answer**.
>
> Example: a quick button may say `Contact`, but after the click, the chat bubble can show `How can I reach Oosu, and what kind of project would make him say yes immediately?` The answer can then render a contact card, CTA buttons, source badges, and a short grounded response.

### 15-1. Core Direction

| Item | v10 | v11 |
| --- | --- | --- |
| Quick question UI | Uses the FAQ question text directly | Separates quick label from natural chat question |
| Question matching | Matches clicked text against FAQ patterns | Passes `faqId` / `intentId` directly for stable cache hits |
| Answer output | Mostly text | Text plus rich visual blocks |
| API cost | Skips Groq on FAQ cache hit | Still skips Groq, but returns visual metadata too |
| RAG structure | Source chunk IDs | Source chunk IDs + renderSpec + mediaRefs |
| Visitor experience | Chatbot-like | Conversational portfolio with interactive profile elements |

### 15-2. Field Definitions

| Field | Meaning |
| --- | --- |
| `quickLabel` | Short label shown on a quick question pill. Example: `Top projects`, `Contact`, `AI workflow`. |
| `displayQuestion` | Natural question shown as the user message after a quick question is clicked. |
| `faqId` | FAQ Answer Cache ID from v10. Used for routing and cache lookup. |
| `intentId` | Question intent. Used for matching and follow-up retrieval hints. |
| `answerText` | Short/default/detailed answer from v10. |
| `renderSpec` | Instructions for how the answer should be rendered visually. |
| `visualBlocks` | Structured blocks such as profile card, project cards, skill chips, timeline, comparison grid, or contact card. |
| `mediaRefs` | References to profile images, project thumbnails, screenshots, or diagrams. Use `TODO_ASSET` when the real file is not ready. |
| `answerParts` | Ordered parts that make up the final assistant message. |

### 15-3. FAQ Click Flow

```text
1. User clicks a quick question.
2. Frontend reads faqId and displayQuestion from the FAQ data.
3. Chat UI adds displayQuestion as the user bubble.
4. /api/chat receives:
   - faqId
   - intentId
   - language
   - displayQuestion
   - originalQuickLabel
5. If faqId is direct-cacheable, the server does not call Groq.
6. The server returns cached answer text plus renderSpec / visualBlocks / mediaRefs.
7. Frontend renders answerParts in order.
8. For follow-up questions, the previous faqId / intentId is passed as a retrieval hint. Use RAG + Groq only when needed.
```

### 15-4. Extended v11 FAQ Answer Schema
...
type FaqRichAnswer = {
  ...
  patterns: string[];
  cacheMode: "direct_cache" | "cache_rewrite" | "rag_required";
  answerSource: "faq_cache" | "faq_rewrite" | "rag_generation" | "fallback";
  skippedGroq: boolean;
...
### 15-5. Answer Parts Rendering Contract

For a v11 rich FAQ hit, `/api/chat` can return structured parts instead of only a single string.
...
```json
{
  ...
  "badge": "Frequently asked answer",
  "answerParts": [
    { "type": "component", "component": "ProfileHeroCard", "dataKey": "profile.summary" },
    { "type": "markdown", "contentKey": "defaultAnswer" },
    { "type": "component", "component": "SkillChipGroup", "dataKey": "skills.core" },
    { "type": "sourceBadges", "sourceChunkIds": ["profile.summary", "profile.strengths"] }
  ]
}
```

The frontend should render known part types safely. Unknown components should be ignored, with markdown fallback preserved.

### 15-6. Rich Visual Block Types

| Block Type | Use Case | UI Example |
| --- | --- | --- |
| `ProfileHeroCard` | Intro, strengths, hire reason | Profile image, name, title, location, core tags, one-line intro |
| `ProjectShowcaseCards` | Top projects, project summary | AskOosu / Instagram Clone / Sticks & Stones cards |
| `ProjectDeepDivePanel` | AskOosu, Instagram Clone, Sticks & Stones detail | Problem, solution, role, tech, lesson |
| `SkillChipGroup` | Tech stack, strengths, AI usage | Frontend / Backend / AI-RAG / Design-UX / Business-Data chips |
| `ExperienceBridgeDiagram` | Business background, OOSU SALON, career transition | CX → UX → Fullstack → AI Service diagram |
| `CareerTimeline` | Career transition or diverse background | Business/Data → OOSU SALON → UX → Fullstack/AI timeline |
| `ComparisonGrid` | Portfoli-Oh! vs AskOosu | Before/after or two-column comparison |
| `AIWorkflowSteps` | AI tool usage | Plan → Generate → Review → Test → Ship steps |
| `ContactCard` | Contact, collaboration, roles | Email, GitHub, LinkedIn, Portfolio buttons |
| `ImageCard` | Project screenshot or interests | Image plus short caption |

### 15-7. Media Reference Rules

| Rule | Meaning |
| --- | --- |
| Prefer real files | Only mark an image as `ready` when the file exists under `/public/images/...`. |
| Use TODO for missing files | Do not invent URLs. Use `src: "TODO_ASSET"` and `status: "todo"`. |
| Avoid external image links | Do not invent external image URLs. Use verified public links only for GitHub/LinkedIn/etc. |
| Alt text required | Every image must include accessible alt text. |
| Visual fallback required | If media is missing, render text/cards/chips without breaking the UI. |

Suggested asset keys:

| Asset Key | Purpose | Suggested Path | Status |
| --- | --- | --- | --- |
| `profile.oosu.portrait` | Profile hero image | `/images/profile/oosu-profile.webp` | TODO_ASSET |
| `project.askoosu.cover` | AskOosu project card | `/images/projects/askoosu-cover.webp` | TODO_ASSET |
| `project.instagram.cover` | Instagram Clone card | `/images/projects/instagram-clone-cover.webp` | TODO_ASSET |
| `project.sticks.cover` | Sticks & Stones card | `/images/projects/sticks-stones-cover.webp` | TODO_ASSET |
| `project.portfoliooh.cover` | Portfoli-Oh! comparison card | `/images/projects/portfolio-oh-cover.webp` | TODO_ASSET |
| `life.oosu.salon` | OOSU SALON / CX image | `/images/life/oosu-salon.webp` | TODO_ASSET |
| `life.sensory.interests` | Hobbies / sensory interests image | `/images/life/sensory-interests.webp` | TODO_ASSET |

### 15-8. Natural Display Question + Rendering Map EN

| FAQ ID | quickLabel | displayQuestion | Recommended Layout | Visual Blocks |
| --- | --- | --- | --- | --- |
| `faq.profile.intro.default` | About Oosu | Who is Oosu, and what kind of developer is he becoming? | `profile_hero` | `ProfileHeroCard`, `SkillChipGroup`, `sourceBadges` |
| `faq.profile.strengths.default` | Strengths | What are Oosu's strongest qualities as a developer? | `profile_hero` | `ProfileHeroCard`, `SkillChipGroup`, `ExperienceBridgeDiagram` |
| `faq.profile.business_to_dev.default` | Business → Dev | How does Oosu's business and customer experience background shape the way he builds products? | `experience_bridge` | `ExperienceBridgeDiagram`, `CareerTimeline` |
| `faq.career.transition.default` | Career transition | How did Oosu move from business, service, and UX into software development? | `timeline` | `CareerTimeline`, `SkillChipGroup` |
| `faq.project.askoosu.overview.default` | AskOosu | What problem led to AskOosu, and why did Oosu build it as a conversational portfolio? | `project_deep_dive` | `ProjectDeepDivePanel`, `AIWorkflowSteps`, `sourceBadges` |
| `faq.project.askoosu.rag.default` | RAG system | How do Notion, RAG, Groq, and PostgreSQL work together inside AskOosu? | `ai_workflow` | `AIWorkflowSteps`, `statelessDiagram`, `sourceBadges` |
| `faq.project.instagram.learned.default` | Instagram Clone | What did building Instagram Clone teach Oosu about fullstack development? | `project_deep_dive` | `ProjectDeepDivePanel`, `SkillChipGroup` |
| `faq.project.sticks.importance.default` | Sticks & Stones | Why does the Sticks & Stones project matter in Oosu's portfolio? | `project_deep_dive` | `ProjectDeepDivePanel`, `ImageCard` |
| `faq.project.portfoliooh_vs_askoosu.default` | Portfolio evolution | How are Portfoli-Oh! and AskOosu different, and why did the direction change? | `comparison_grid` | `ComparisonGrid`, `ProjectShowcaseCards` |
| `faq.career.oosu_salon_to_dev.default` | OOSU SALON → UX | How did running OOSU SALON connect to UX and development? | `experience_bridge` | `ExperienceBridgeDiagram`, `ImageCard`, `CareerTimeline` |
| `faq.profile.collaboration.default` | Collaboration | What is it like to work with Oosu on a team? | `profile_hero` | `ProfileHeroCard`, `timeline`, `sourceBadges` |
| `faq.ai_usage.workflow.default` | AI workflow | How does Oosu actually use tools like Claude Code, Codex, and Gemini in development? | `ai_workflow` | `AIWorkflowSteps`, `SkillChipGroup` |
| `faq.career.target_role.default` | Target role | What kind of role or project would make Oosu say yes? | `contact_card` | `ContactCard`, `SkillChipGroup`, `ctaButtons` |
| `faq.link.resume.default` | Resume | Is Oosu's resume or detailed career profile ready to share? | `contact_card` | `ContactCard`, `sourceBadges` |
| `faq.link.live_url.default` | Live links | Which portfolio or project links can I open right now? | `contact_card` | `ContactCard`, `ProjectShowcaseCards` |
| `faq.contact.collaboration.default` | Contact | How can I reach Oosu, and what kind of collaboration is he open to? | `contact_card` | `ContactCard`, `ctaButtons` |
| `faq.fun.interests.default` | Outside code | What does Oosu care about outside code, and how does that influence his perspective? | `profile_hero` | `ImageCard`, `SkillChipGroup`, `ExperienceBridgeDiagram` |
| `faq.profile.growth_areas.default` | Growth areas | What is Oosu actively improving about himself as a developer? | `timeline` | `CareerTimeline`, `sourceBadges` |
| `faq.project.top_three.default` | Top projects | Can you show Oosu's top three projects at a glance? | `project_showcase` | `ProjectShowcaseCards`, `SkillChipGroup` |
| `faq.skills.tech_stack.default` | Tech stack | What technologies does Oosu use, and where has he applied them? | `skill_cloud` | `SkillChipGroup`, `ProjectShowcaseCards` |
| `faq.career.hire_reason.default` | Why Oosu | What strengths could a team expect if Oosu joined? | `profile_hero` | `ProfileHeroCard`, `ExperienceBridgeDiagram`, `ProjectShowcaseCards` |
| `faq.career.salon_close.default` | OOSU SALON story | How did closing OOSU SALON influence Oosu's next career direction? | `timeline` | `CareerTimeline`, `ExperienceBridgeDiagram` |
| `faq.profile.language.default` | Languages | What languages can Oosu use professionally? | `profile_hero` | `SkillChipGroup`, `sourceBadges` |
| `faq.profile.focus.default` | Diverse background | Does Oosu's diverse background make his direction broader or more focused? | `experience_bridge` | `ExperienceBridgeDiagram`, `CareerTimeline` |

### 15-9. Rich FAQ Examples

#### Example A — Top Projects
...
#### Example B — Contact / Collaboration

```ts
{
  id: "faq.contact.collaboration.default",
  quickLabel: "Contact",
  displayQuestion: "How can I reach Oosu, and what kind of collaboration is he open to?",
  cacheMode: "direct_cache",
  skippedGroq: true,
  renderSpec: {
    layout: "contact_card",
  ...
  visualBlocks: [
    ...
    },
    {
      type: "ctaButtons",
      items: [
        { "label": "Email", "hrefKey": "contact.email" },
        { "label": "GitHub", "hrefKey": "contact.github" },
        { "label": "LinkedIn", "hrefKey": "contact.linkedin" },
        { "label": "Portfolio", "hrefKey": "contact.portfolio" }
      ]
    }
  ],
  guardrails: ["Do not invent resume URLs", "Do not invent employment availability", "Do not expose private details"]
}
```
...
| Situation | Behavior |
| --- | --- |
| User clicks a quick question | Show `displayQuestion` as the user bubble |
| User types manually | Show the user's original text unchanged |
| quickLabel and displayQuestion differ | Log both for analytics |
| Direct FAQ cache hit | Render `answerParts` without calling Groq |
| Missing media asset | Hide the image block and use text/card fallback |
| Low confidence | Avoid heavy rich visuals and show a cautious badge |
| TODO included | Show the confirmation-needed badge |

### 15-11. Implementation Priority

1. Add `quickLabel`, `displayQuestion`, `renderSpec`, `visualBlocks`, and `mediaRefs` to `data/faq-answers.ko.ts` and `data/faq-answers.en.ts`.
2. When a quick question is clicked, add `displayQuestion` to the chat as the user message, not the raw button label.
3. If `/api/chat` receives `faqId`, treat it as a high-confidence FAQ hit or bypass text matching safely.
4. Include `answerParts` in FAQ cache responses.
5. Build a `RichAnswerRenderer` that renders `answerParts` in order.
6. Implement text + cards + chips + source badges first. Connect images only when real assets exist.
7. Expand later with `ProjectDeepDivePanel`, `ComparisonGrid`, and `AIWorkflowSteps`.

### 15-12. v11 Quality Checklist

- [ ] Existing FAQ IDs and intent IDs are preserved.
- [ ] No new dedicated `Me / Projects / Skills / Fun / Contact` knowledge sections are created.
- [ ] Quick question clicks show natural `displayQuestion` text in the chat.
- [ ] FAQ cache hits do not call Groq.
- [ ] Answers can render rich visual blocks, not only plain text.
- [ ] Missing image files do not break the UI.
- [ ] The system does not invent TODO links, resume URLs, private repos, or performance metrics.
- [ ] Korean and English FAQ items each have language-matched `displayQuestion` values.
- [ ] Analytics/logging stores `quickLabel`, `displayQuestion`, `faqId`, `intentId`, `answerSource`, and `skippedGroq`.
