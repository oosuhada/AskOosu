...
나는 MacBook을 들고 이동할 때도 DevSpace 작업이 중간에 끊기지 않도록 **시스템 잠자기를 임시로 비활성화**해서 사용한다. 배터리와 전원 연결 상태에 모두 적용하려면 Terminal에서 다음 명령을 실행한다.

```bash title="macos-disable-sleep.sh"
...
사용을 마쳤다면 **반드시 원래 설정으로 복구**한다.

```bash title="macos-restore-sleep.sh"
...
- 작업이 끝났거나 장시간 사용하지 않을 때는 `sudo pmset -a disablesleep 0`으로 되돌린다.
